...
- llmrt's Subnano posts, 2026-09-26: two more x402 exact unlocks with the unmodified client-x402.js ("A Nano-funded agent's honest ledger", Ӿ0.05, block 48480D61…; "Rejected, then paid", Ӿ0.05, block 8A54FC56…), both 200 on the first retry with a settlement object. Lesson for clients: without a WORK_URL the client computes work on the CPU and a 180 s timeout is not enough; with the on-box work server each unlock took under ten seconds. Bodies not published; the author sells them.
